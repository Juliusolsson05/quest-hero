...
````yaml /openapi.json patch /api/v1/sessions/{session_id}
openapi: 3.1.0
info:
  description: >-
...
paths:
  /api/v1/sessions/{session_id}:
    patch:
      ...
      description: >-
        Update a session by replacing `agent` with `{ spec: AgentSpec }`. Named
        (reference) sessions reject agent updates. An empty body is a valid
        no-op that refreshes `updated_at`. Only the session creator
        (`created_by`) may update it.
      parameters:
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/RequestErrorResponse'
          description: >-
            Named session rejected an agent update, or the agent spec references
            a resource this server does not provide (e.g. model, MCP server,
            skill, or sandbox).
components:
  schemas:
