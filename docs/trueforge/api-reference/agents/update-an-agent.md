...
````yaml /openapi.json put /api/v1/agents/{agent_id}
openapi: 3.1.0
info:
  description: >-
...
paths:
  /api/v1/agents/{agent_id}:
    put:
      ...
      description: >-
        Replaces the manifest for an existing agent keyed by immutable
        `agent_id`.
      parameters:
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/RequestErrorResponse'
          description: >-
            The agent spec is valid but requires a capability this server does
            not provide (e.g. sandbox or skills).
components:
  schemas:
    UpdateAgentRequest:
