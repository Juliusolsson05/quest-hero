...
````yaml /openapi.json post /api/v1/settings/mcp-servers
openapi: 3.1.0
info:
  description: >-
...
paths:
  /api/v1/settings/mcp-servers:
    post:
      ...
      description: >-
        Creates an MCP server by `name`. Fails if `name` is already taken. Runs
        DCR registration when `auth.type` is `dcr`. Header secrets: real value
        required; redacted with no stored value returns 400.
      requestBody:
        content:
          application/json:
            schema:
