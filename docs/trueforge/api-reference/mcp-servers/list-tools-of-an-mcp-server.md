...
````yaml /openapi.json get /api/v1/mcp-servers/{name}/tools
openapi: 3.1.0
info:
  description: >-
...
paths:
  /api/v1/mcp-servers/{name}/tools:
    get:
      ...
      description: >-
        All tools exposed by the given MCP server (non-paginated), as returned
        by the MCP `tools/list` call.
      parameters:
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/RequestErrorResponse'
          description: >-
            The MCP server requires authentication (does not trigger browser
            OIDC login).
        '502':
          content:
            application/json:
  ...
  schemas:
    ListMCPServerToolsResponse:
      properties:
        data:
          description: >-
            MCP `tools/list` entries, passed through verbatim from the MCP
            server.
          items:
