 ...
 *   lens.ts       how the question reaches the hub, which owns the agent
 *   index.ts      the viewfinder, the shutter, and the card (this file)
 */
...
export class PhotoMode {
  ...
  /** Clock and sky, forwarded to the agent so a dossier can talk about the
   *  light the photograph was taken in. */
  ...
  private paintCard(): void {
    ...
    if (shot.state === 'failed') {
      ...
      // Only claim the source once something has actually arrived — until the
      // first byte lands we do not know that the archivist answered at all.
      const via = shot.tools.length || shot.raw
        ? 'live from World Hub → TrueForge · ashford-lens · sf-guide'
        : 'reaching the archivist…';
