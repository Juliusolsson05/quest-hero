/**
 * The Lens — photo mode's field archivist.
 *
 * A photograph arrives as a subject (a real San Francisco building, resolved
 * client-side in game/src/landmarks.ts) and leaves as a dossier: a caption and
 * three or four facts, each fetched live and each carrying the dataset it came
 * from. Nothing here is written in advance — the agent is handed the `sf-guide`
 * MCP server (32 live SF sources: DataSF, NOAA, USGS, Open-Meteo, Wikipedia)
 * and told to go and look.
 *
 * A named agent in the Library like every character (`sf-quest-lens`), with one
 * session per subject id, so photographing the same building twice reaches the
 * archivist who already saw your first shot.
 *
...
const LENS_DEF: AgentDef = {
  registryName: 'sf-quest-lens',
  model: 'openai/gpt-5-5',
  // One building, a handful of dataset queries: nothing to fan out to, and a
  // subagent's findings would land outside the bubble the card is reading.
  subagents: false,
  // The player is standing in the street holding a camera: a handful of dataset
  // lookups and a caption do not need deep reasoning, and every second of it is
  // a second of watching a spinner.
  params: { reasoningEffort: 'low', parallelToolCalls: true },
  // Named rather than `webAccess`, which would also hand a photographer the
  // stock-market server. Tavily is the identity backstop the persona allows.
  connectors: ['sf-guide', 'tavily'],
  persona:
    ...
    'have it than a plausible sentence. Quote years and counts as plain integers: a dataset that ' +
    'stores 1984.0 means 1984.\n' +
    '3. Prefer the sf-guide sources. Use web search only to confirm a building’s identity when ' +
    'the city datasets have nothing, and label it as web.\n\n' +
...
/** The turn itself: one photograph, described. The game's clock and weather
 *  are the hub's to know, so they are added here rather than trusted to the
 *  client that took the picture. */
function shotBrief(shot: Shot): string {
  ...
  return [
    ...
    `subject: ${shot.name}`,
    shot.where ? `address: ${shot.where}` : '',
    shot.district ? `district: ${shot.district}` : '',
    `coordinates: ${shot.lat.toFixed(4)}, ${shot.lon.toFixed(4)}`,
    `framing: ${shot.distanceM}m away, facing ${shot.bearing}, ${shot.focalMm}mm`,
    `in the game right now: ${hh}:${mm}, ${w.kind} ${Math.round(w.tempC)}°C ` +
      '(the game mirrors real San Francisco weather and time)',
    shot.sources.length ? `sources worth trying first: ${shot.sources.join(', ')}` : '',
    '',
